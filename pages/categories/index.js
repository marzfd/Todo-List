        ...
        <div className='mx-auto p-4 md:p-8 md:w-1/2 lg:w-1/2 w-full self-center'>
          <ul className='space-y-3'>
            {categories.map((category, index) => (
              <Link href={`/tasks/${category.category_id}`} key={index}>
                <a>
                  <li
          ...
          <Link href='/tasks'>
                <a>
                  <button className='w-full mt-5 bg-purple-700 hover:bg-purple-500 transition duration-300 text-white lg:text-sm py-2 px-4 rounded-lg shadow-lg shadow-purple-500/50 focus:outline-none'>
                    See All Tasks
                  </button>
                </a>
              </Link>
